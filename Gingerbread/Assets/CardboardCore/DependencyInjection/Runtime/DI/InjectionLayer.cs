		...
		public void ReleaseDependencies(object @object)
		{
			...
			for (int i = 0; i < instancesToDestroy.Count; i++)
			{
				...
				if (instancesToDestroy[i].GetType().IsSubclassOf(typeof(MonoBehaviour)))
				{
					...
					// Added try catch to allow the system to keep going without throwing errors and without having to fiddle
					// with order of execution for some scripts
					if (Application.isPlaying)
					{
						try
						{
						...
						{
							// GameObject has already been destroyed, continue with the loop
							continue;
						}
					}
				}
